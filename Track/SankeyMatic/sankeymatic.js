...
(function sankeymatic(glob) {
  ...
  function updateResetNodesUI() {
    ...
    el('reset_all_moved_nodes').disabled = false
  }
  ...
  function initializeDiagram(cfg) {
    ...
    // Check the current preference and set the background color
    if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
      cfg.bg_color = "#000"; // Dark mode
    } else {
        cfg.bg_color = "#FFF"; // Light mode, assuming default is white
    }
  ...
  function render_sankey(allNodes, allFlows, cfg, numberStyle) {
    ...
    function measureSVGText(txtList, id) {
      ...
      const txtId = `bb_${id}`, // (bb for 'BoundingBox')
        [xC, yC] = [window.innerWidth / 2, window.innerHeight / 2], // centers
        textEl = scratchRoot
          .append('text')
          .attr('id', txtId)
          .attr('x', ep(xC))
    ...
    function setUpDiagramSize() {
      ...
      return {
        w: finalW,
        h: graphH,
        final_margin_l: leadingW + leadingCutOffAdjustment,
      };
    }
    ...
    // Add a [g]roup translating the remaining elements 'inward' by the margins:
      ...
      .attr('transform', `translate(${(window.innerWidth - graph.final_margin_l - graph.w) / 2}, ${(window.innerHeight - graph.h) / 2})`);
  ...
  glob.process_sankey = () => {
    ...
    skmSettings.forEach((fldData, fldName) => {
      const [dataType, defaultVal] = fldData,
        ...
        sizeObj = dataType === 'contained'
          ? { w: window.innerWidth, h: approvedCfg.innerHeight}
          : {},
    ...
    const chartEl = el('chart');
    chartEl.style.height = `${window.innerHeight}px`;
    chartEl.style.width = `${window.innerWidth}px`;
  
    // Also update the PNG download buttons' title text with these dimensions:
    [1, 2, 4, 6].forEach((s) => {
      el(`save_as_png_${s}x`).title
        = `PNG image file: ${window.innerWidth * s} x ${window.innerHeight * s}`;
    });
  ...
  /* global
   ...
   skmSettings colorGray60 "flows_in" breakpointField
   ...
   function updateDimensions() {
    // Trigger any onchange or related updates
